...
/// <summary>A <see cref="Result"/> that carries a payload on success.</summary>
public record Result<T>(bool Ok, string Status, T? Value)
{
    public static Result<T> Success(T value, string status = "Ok") => new(true, status, value);
    public static Result<T> Fail(string status) => new(false, status, default);
}
