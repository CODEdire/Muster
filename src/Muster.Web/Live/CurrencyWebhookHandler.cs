...
/// subscriber can't block the others or trigger mass redelivery.
///
/// <para>Lives in the web host (not the bot) because this is outbound HTTP work — no Discord gateway needed,
/// and if web ever scales horizontally the topic's web subscription gives free competing-consumer load
/// balancing across web instances. The bot still subscribes to the same topic for its own purpose
/// (<c>CurrencyDmHandler</c>, the Discord DM receipt) — each host has its own subscription on
/// <c>muster.currency-movement-recorded</c> and runs only the handlers in its own assembly.</para>
/// </summary>
