...
/// <summary>Suggests the guild's admin-managed shop types (by name) — value is the store-type id.</summary>
public class ShopStoreTypeAutocompleteProvider(IServiceScopeFactory scopeFactory)
    : IAutocompleteProvider<AutocompleteInteractionContext>
{
    public async ValueTask<IEnumerable<ApplicationCommandOptionChoiceProperties>> GetChoicesAsync(
        ApplicationCommandInteractionDataOption option, AutocompleteInteractionContext context)
    {
        using var _ = BotTelemetry.MeasureAutocomplete(nameof(ShopStoreTypeAutocompleteProvider));
        ...
        var types = await db.ShopStoreTypes
