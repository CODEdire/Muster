...
/// bus, so they run inside the transactional/outbox pipeline. They delegate to <see cref="CurrencyService"/>
...
{
    public static async Task<CurrencyChangeResult> Handle(MintCurrency command, CurrencyService currency, CancellationToken ct)
    {
...
{
    public static async Task<CurrencyChangeResult> Handle(SpendCurrency command, CurrencyService currency, CancellationToken ct)
    {
