...
/// <summary>The signed-in user's profile for the shell (display name + @username handle + optional avatar URL).</summary>
public record UserProfile(string Name, string Username, string? AvatarUrl);
...
public class WebGuildService(MusterDbContext db, GuildAuthorizationService auth, ICurrencyReadService scores)
{
    ...
    public async Task<UserProfile?> GetUserProfileAsync(ulong userId, CancellationToken ct = default)
    {
        ...
        return user is null ? null : new UserProfile(user.GlobalName ?? user.Username, user.Username, Discord.DiscordCdn.AvatarUrl(user.Id, user.AvatarHash));
    }
