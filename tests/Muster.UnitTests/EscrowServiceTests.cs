...
public class EscrowServiceTests
{
    ...
    private static async Task FundAsync(MusterDbContext db, AwardService awards, Currency coin, ulong userId, long amount)
        => await awards.AwardAsync(1, userId, coin.Id, amount, LedgerSourceType.Connector, null, "seed");
    ...
    public async Task Hold_DebitsOwner_CreditsEscrow_AndCommitsWithStateInOneTransaction()
    {
        ...
        var sut = new EscrowService(db, awards);
        ...
        Assert.Equal(40, await BalanceAsync(db, EscrowService.EscrowAccountUserId, coin.Id));
    }
    ...
    public async Task Hold_Then_Payout_TransfersToCompleter_ZeroSum()
    {
        ...
        Assert.Equal(0, await BalanceAsync(db, EscrowService.EscrowAccountUserId, coin.Id)); // escrow emptied
    }
    ...
    public async Task Hold_Then_Refund_RestoresOwner()
    {
        ...
        Assert.Equal(0, await BalanceAsync(db, EscrowService.EscrowAccountUserId, coin.Id));
    }
